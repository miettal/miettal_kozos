...
static struct connection *tcp_search_connection_from_addr(uint32 src_ipaddr,
							  uint32 dst_ipaddr,
							  uint16 src_port,
							  uint16 dst_port)
{
  ...
  while (1) {
    kz_recv(MSGBOX_ID_TCPCONLIST, NULL, (void *)&con);
    if (con == NULL) { /* 終端まできた */
...
{
  struct pseudo_header { /* TCPチェックサム計算用の疑似ヘッダ */
    uint32 src_ipaddr;
    uint32 dst_ipaddr;
    uint8  zero;
    uint8  protocol;
  ...
  phdr.src_ipaddr = hton4(src_ipaddr);
  phdr.dst_ipaddr = hton4(dst_ipaddr);
  phdr.zero = 0;
  phdr.protocol = protocol;
  phdr.size = hton2(size);
...
static int tcp_sendpkt(struct netbuf *pkt, struct connection *con)
{
  ...
  tcphdr->seq_number = hton4(con->seq_number);
  tcphdr->ack_number = hton4(con->ack_number);

  size = 0;
  if (tcphdr->flags & TCP_HEADER_FLAG_PSH) {
    size = pkt->size - hdrlen;
  } else if (tcphdr->flags & (TCP_HEADER_FLAG_SYN|TCP_HEADER_FLAG_FIN)) {
    size = 1;
  }
  ...
  tcphdr->checksum = hton2(tcp_calc_pseudosum(con->src_ipaddr, con->dst_ipaddr,
					      IP_PROTOCOL_TCP, pkt->size));
  tcphdr->checksum = hton2(ip_calc_checksum(pkt->size, tcphdr));
				  ...
				  int size, char *data)
{
  ...
  tcphdr->src_port   = hton2(con->src_port);
  tcphdr->dst_port   = hton2(con->dst_port);
  tcphdr->flags      = flags;
  tcphdr->window     = hton2(window);
  tcphdr->urgent     = 0;
  ...
  if (mss) {
    ...
    *(uint16 *)option = hton2(mss); option++; option++;
  }
  if (window_scale) {
			   ...
			   int size, char *data)
{
  ...
  if (pkt) return tcp_sendpkt(pkt, con);
  return 0;
}
...
static int tcp_recv_flush(struct connection *con)
{
  ...
  for (pkt = con->recv_queue; pkt; pkt = next) {
    ...
    next = pkt->next;
    if (ntoh4(tcphdr->seq_number) != con->ack_number) {
      prevp = &(pkt->next);
      continue;
    }
    ...
    *prevp = next;
    ...
    pkt->top  += hdrlen;
    pkt->size -= hdrlen;
    con->ack_number = ntoh4(tcphdr->seq_number) + pkt->size;
...
static int tcp_recv(struct netbuf *pkt)
{
  ...
  con = tcp_search_connection_from_addr(0,
					...
					ntoh2(tcphdr->dst_port),
					ntoh2(tcphdr->src_port));
  if (!con)
    con = tcp_search_connection_from_addr(0, 0, ntoh2(tcphdr->dst_port), 0);
  ...
  if (tcphdr->flags & TCP_HEADER_FLAG_ACK) {
    con->seq_number = ntoh4(tcphdr->ack_number);
  ...
  if (tcphdr->flags & TCP_HEADER_FLAG_SYN) {
    ...
    if (con->status == TCP_CONNECTION_STATUS_LISTEN) {
      ...
      con->dst_port   = ntoh2(tcphdr->src_port);
      con->ack_number = ntoh4(tcphdr->seq_number) + 1;
...
int tcp_main(int argc, char *argv[])
{
  ...
  while (1) {
    kz_recv(MSGBOX_ID_TCPPROC, NULL, (void *)&buf);
    ret = tcp_proc(buf);
