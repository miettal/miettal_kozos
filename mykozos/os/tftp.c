		      ...
		      char *filename)
{
  ...
  tftphdr->request.opcode = hton2(TFTP_OPCODE_READ);
		     ...
		     int block_number)
{
  ...
  tftphdr = (union tftp_header *)buf->top;
  tftphdr->ack.opcode = hton2(TFTP_OPCODE_ACK);
  tftphdr->ack.block_number = hton2(block_number);
