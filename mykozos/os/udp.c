...
static struct portinfo *udp_search_portinfo_from_addr(uint32 src_ipaddr,
						      uint32 dst_ipaddr,
						      uint16 src_port,
						      uint16 dst_port)
{
  ...
  while (1) {
    kz_recv(MSGBOX_ID_UDPPORTLIST, NULL, (void *)&pinfo);
    if (pinfo == NULL) { /* 終端まできた */
...
static int udp_sendpkt(struct netbuf *pkt, uint32 src_ipaddr, uint32 dst_ipaddr)
{
  ...
  udphdr->checksum = hton2(udp_calc_pseudosum(src_ipaddr, dst_ipaddr,
					      IP_PROTOCOL_UDP, pkt->size));
  udphdr->checksum = hton2(ip_calc_checksum(pkt->size, udphdr));
				  ...
				  int size, char *data)
{
  ...
  udphdr->src_port = hton2(src_port);
  udphdr->dst_port = hton2(dst_port);
  udphdr->length   = hton2(hdrlen + size);
...
static int udp_recv(struct netbuf *pkt)
{
  ...
  pinfo = udp_search_portinfo_from_addr(0,
					...
					ntoh2(udphdr->dst_port),
					ntoh2(udphdr->src_port));
  if (!pinfo)
    pinfo = udp_search_portinfo_from_addr(0, 0, ntoh2(udphdr->dst_port), 0);
  ...
  pkt->option.udp.recv.port = ntoh2(udphdr->src_port);
...
int udp_main(int argc, char *argv[])
{
  ...
  while (1) {
    kz_recv(MSGBOX_ID_UDPPROC, NULL, (void *)&buf);
    ret = udp_proc(buf);
